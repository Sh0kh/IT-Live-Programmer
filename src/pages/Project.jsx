...
function Project() {
    ...
    const { data: getAllTasks, loading, error } = useQuery(
        employeeRole === 'TEAM_LEADER' ? GET_TEAM_LEADER_TASK : GET_TASKS,
        {
            variables: employeeRole === 'TEAM_LEADER' ? { ID } : { employeeId },
            skip: !employeeId && employeeRole !== 'TEAM_LEADER',
        }
    );
...
useEffect(() => {
  ...
  const showErrorToast = () => {
    ...
    const createAndSendTask = (taskId, status) => {
        ...
        if (status === 'CONFIRMED' && ['FRONT_END', 'BACK_END', 'DESIGNER', 'PROJECT_MANAGER'].includes(employeeRole)) {
            // alert('У вас нет прав на подтверждение этой задачи.');
    ...
    const handleDrop = (event, targetColumn) => {
        ...
        if (newStatus === 'CONFIRMED' && ['FRONT_END', 'BACK_END', 'DESIGNER', 'PROJECT_MANAGER'].includes(employeeRole)) {
            showErrorToast();
            return; // Прерываем выполнение функции, чтобы дроп не произошел
        }
